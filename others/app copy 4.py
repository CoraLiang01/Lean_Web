...
# $$
# \max \quad \sum_{(l,k,j)} \text{avg\_price}[(l,k,j)] \cdot x[(l,k,j)]
# $$
...
# - Each Eco_flex ticket consumes 1.2 units of capacity, each Eco_lite ticket consumes 1 unit.

# For all $(l,k)$ in the selected flights:
# $$
# 1.2 \cdot x[(l,k,f)] + 1 \cdot x[(l,k,l)] \leq 187
# $$
...
# For each OD pair $l$:
# $$
# \text{ratio\_0\_list}[l] \cdot x_o[l] + \sum_{k,j} \text{ratio\_list}[(l,k,j)] \cdot x[(l,k,j)] = \text{avg\_pax}[l]
# $$
...
# $$
# \frac{x[(l,k,j)]}{\text{value\_list}[(l,k,j)]} - \frac{x_o[l]}{\text{value\_0\_list}[l]} \leq 0
# $$
...
# For each ticket option \((l,k,j)\):
# $$
# x[(l,k,j)] \leq 10^7 \cdot y[(l,k)]
# $$

# #### 5. Cardinality Constraint

# Select at most 2 flights:
# $$
...
# At each airport, the number of selected inflow flights equals the number of selected outflow flights:

# - For airport A:
# $$
# \sum_{(l,k) \in \text{sigma\_inflow\_A}} y[(l,k)] = \sum_{(l,k) \in \text{sigma\_outflow\_A}} y[(l,k)]
# $$

# - For airport B:
# $$
# \sum_{(l,k) \in \text{sigma\_inflow\_B}} y[(l,k)] = \sum_{(l,k) \in \text{sigma\_outflow\_B}} y[(l,k)]
# $$

# - For airport C:
# $$
# \sum_{(l,k) \in \text{sigma\_inflow\_C}} y[(l,k)] = \sum_{(l,k) \in \text{sigma\_outflow\_C}} y[(l,k)]
# $$
...
# }
# value_0_list = {'AB': 0.022767048, 'AC': 0.024033093, 'BA': 0.133469692, 'CA': 0.126816434}
# ratio_0_list = {'AB': 0.002224984, 'AC': 1.0, 'BA': 0.012916147, 'CA': 1.0}
# avg_pax = {'CA': '4807.43', 'AB': '38965.86', 'BA': '33210.71', 'AC': '4812.5'}
# capacity_consum = 1.2
# option_num = 2
...
# # Set of all (l,k) pairs (flight legs)
# flight_legs = set()
# for key in avg_price.keys():
#     l, k, j = key.strip('()').split(',')
#     flight_legs.add(f"({l},{k})")
# flight_legs = list(flight_legs)

# model = gp.Model("sales_based_lp")
# y = model.addVars(flight_legs, vtype=GRB.BINARY, name="y")
# x = model.addVars(avg_price.keys(), lb=0, name="x")
# x_o = model.addVars(value_0_list.keys(), lb=0, name="x_o")

# # Objective
# model.setObjective(gp.quicksum(float(avg_price[key]) * x[key] for key in avg_price.keys()), GRB.MAXIMIZE)

# # Capacity constraints
# for l_k in flight_legs:
#     l, k = l_k.strip('()').split(',')
#     key_f = f"({l},{k},f)"
#     key_l = f"({l},{k},l)"
#     if key_f in avg_price and key_l in avg_price:
#         model.addConstr(
#             capacity_consum * x[key_f] + x[key_l] <= flight_capacity,
#             name=f"capacity_{l_k}"
#         )
...
#     l = key.split(',')[0].strip('(')
#     model.addConstr(
#         x[key] / value_list[key] - x_o[l] / value_0_list[l] <= 0,
#         name=f"scale_{key}"
#     )
...
# model.addConstr(
#     gp.quicksum(y[l_k] for l_k in flight_legs) <= option_num,
#     name="cardinality"
# )

# # Flow conservation constraints
# model.addConstr(
#     gp.quicksum(y[lk] for lk in \sigma_A^+) == gp.quicksum(y[lk] for lk in \sigma_A^-),
#     name="flow_A"
# )
# model.addConstr(
#     gp.quicksum(y[lk] for lk in \sigma_B^+) == gp.quicksum(y[lk] for lk in \sigma_B^-),
#     name="flow_B"
# )
# model.addConstr(
#     gp.quicksum(y[lk] for lk in \sigma_C^+) == gp.quicksum(y[lk] for lk in \sigma_C^-),
#     name="flow_C"
# )
...
# **Note:** All required parameters and sets are included above. The model will select the optimal 2 flights (from the specified options) that maximize ticket sale revenue, subject to all operational and flow constraints.'''
...
$$
x[(l,k,j)]/v[(l,k,j)] - x_o[l]/v_0[l] \leq 0
$$
